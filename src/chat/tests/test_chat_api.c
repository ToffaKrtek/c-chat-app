...
void setup(void) { storage = chat_storage_new_memory(); }
void teardown(void) { chat_storage_free(storage); }
...
Test(chat_api, chat_has_unread_messages_empty, .init = setup, .fini = teardown) {
  chat_t *chat = chat_create(storage, "test");
...
Test(chat_api, create_message, .init = setup, .fini = teardown) {
  chat_t *chat = chat_create(storage, "test");
...
Test(chat_api, chat_has_unread_messages_no_empty_false, .init = setup, .fini = teardown) {
  chat_t *chat = chat_create(storage, "test");
  chat_message_t *message =
      chat_create_message(storage, true, chat->id, "test");
  bool res = chat_has_unread_messages(storage, chat->id);
...
Test(chat_api, delete_message_no_chat_error, .init = setup, .fini = teardown) {
  int res = chat_delete_message(storage, 999, 999);
  cr_assert(res != EXIT_SUCCESS);
}

Test(chat_api, delete_message_no_msg_error, .init = setup, .fini = teardown) {
  chat_t *chat = chat_create(storage, "test");
...
Test(chat_api, delete_message, .init = setup, .fini = teardown) {
  chat_t *chat = chat_create(storage, "test");
  chat_message_t *message =
      chat_create_message(storage, true, chat->id, "test");
